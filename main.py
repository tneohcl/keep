...
class MainWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        # At the top of the sidebar, as wide as the cards under it: it switches
        # the sidebar and the page together.
        self.view_switch = ViewSwitch(["Status", "Restore"], accessible_name="Keep view", fill=True)
        ...
        # (16 px), so it spans exactly their cards' width.
        sidebar_head = QWidget()
        sidebar_head.setObjectName("keepSidebarHead")
        sidebar_head.setAttribute(Qt.WA_StyledBackground, True)
        head = QVBoxLayout(sidebar_head)
        head.setContentsMargins(16, 16, 16, 0)
        head.addWidget(self.view_switch)
        sidebar = QWidget()
        sidebar.setFixedWidth(300)
        column = QVBoxLayout(sidebar)
        column.setContentsMargins(0, 0, 0, 0)
        column.setSpacing(0)
        column.addWidget(sidebar_head)
        column.addWidget(self.sidebar_stack, 1)
