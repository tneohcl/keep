# Keep Backup: the detailed guide

The long-form reference: configuration, destinations, restore and unlocking in depth. For an overview, start with the [README](../README.md). Keep Backup is called "Keep" for short throughout.
...
### Application selection and the sidebar

The left sidebar holds what's backed up (folders, applications, system settings), the destination, the schedule and Recovery access. The Status | Restore switch at its top changes both the sidebar and the page; **Back up now** is in the toolbar.
...
The main window starts on **Status**, showing backup health, last/next backup, destination and whether you can get your files back. **Restore** lists the stored backups with safe recovery controls; Compare and Delete are in its **Manage** menu. Ctrl+1/Ctrl+2 switch views, Ctrl+B backs up now, Ctrl+R reviews a restore, Ctrl+L shows the log and F5 refreshes status.
...
A **Help** menu in the top-left covers the same ground as this file, in-app: what Safe vs Direct restore do, what "Remember this passphrase" means, and where the disaster-recovery runbook lives. **About Keep Backup** shows the currently active destination, repo path, config file, and Borg's own reported version — useful for sanity-checking which build/destination is actually live.
