...
class SafeRestoreTests(unittest.TestCase):
    ...
    def test_existing_destination_is_never_overwritten(self):
        ...
        done, failed = restore_entries([(self.src, 'file', 'File')], dest)
    ...
    def test_duplicate_targets_report_failure_without_overwriting(self):
        ...
        done, failed = restore_entries([(self.src, 'file', 'A'), (other, 'file', 'B')], dest)
    ...
    def test_missing_requested_source_is_failure(self):
        done, failed = restore_entries([(self.root / 'missing', 'file', 'Missing')], self.root / 'restore')
        self.assertFalse(done)
        self.assertIn('missing', failed[0].lower())

    def test_archive_paths_preserve_equal_basenames(self):
        done, failed = restore_entries([(self.src, 'one/file', 'One'), (self.src, 'two/file', 'Two')], self.root / 'restore')
    ...
    def test_cancel_during_file_reports_incomplete(self):
        ...
        def cancel():
            calls.append(1)
            return len(calls) >= 3
        done, failed = restore_entries([(self.src, 'file', 'File')], self.root / 'restore', cancel)
        self.assertFalse(done)
        self.assertIn('cancelled', failed[0])

    def test_traversal_rejected(self):
        done, failed = restore_entries([(self.src, '../escape', 'File')], self.root / 'restore')
        self.assertFalse(done)
        self.assertTrue(failed)
    ...
    def test_links_preserved_but_never_followed_at_destination(self):
        ...
        dest = self.root / 'restore'
        done, failed = restore_entries([(link, 'link', 'Link'), (self.src, 'link/file', 'File')], dest)
        self.assertEqual((len(done), len(failed)), (1, 1))
...
class RepositoryIdentityTests(unittest.TestCase):
    def test_recreated_repository_does_not_inherit_success(self):
        ...
        self.assertEqual(recovery_test.status(record, '/same/path')[0], 'never')
